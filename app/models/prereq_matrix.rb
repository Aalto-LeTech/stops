    ...
    Skill.transaction do
      ...
      while row < @row_count
        ...
        if course
          ...
          description.save
          #CourseDescription.create(:course_code => code, :locale => @locale, :name => name)
        end
        ...
        begin
          ...
          unless skill
            skill = Skill.create(:course_code => code, :credits => skill_credits.gsub(',','.').to_f, :level => level.to_i, :position => skill_position)
      ...
      for row in 6...@row_count
        for col in 8...@col_count
          ...
          if relation_type
            ...
            # Add course prereq
            course_prereq = course_relations[@skills[col].course_code + @prereq_skills[row].course_code]
            if course_prereq.nil? || (course_prereq == SUPPORTING_PREREQ && relation_type == STRICT_PREREQ)
              # Insert if it does not exist
              p = CoursePrereq.find(:first, :conditions => {:course_id => @skills[col].course.id, :prereq_id => @prereq_skills[row].course.id})
              if p.nil? || p.requirement == SUPPORTING_PREREQ && relation_type == STRICT_PREREQ
                CoursePrereq.delete_all(["course_id = ? AND prereq_id = ?", @skills[col].course.id, @prereq_skills[row].course.id])
                CoursePrereq.create(:course_id => @skills[col].course.id, :prereq_id => @prereq_skills[row].course.id, :requirement => relation_type)
              end
              
              # Make a note that this relation has been added
              course_relations[@skills[col].course_code + @prereq_skills[row].course_code] = relation_type
            end
