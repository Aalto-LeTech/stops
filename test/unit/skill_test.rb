...
class SkillTest < ActiveSupport::TestCase
  ...
  context "A skill" do
    ...
    setup do 
      ...
      course      = FactoryGirl.create :scoped_course

      course.curriculum     = @competence
      competence.curriculum = @competence
      course.save!
      competence.save!
      @root_skill_course_id = course.id

      @skill.competence_nodes << competence
      @skill.competence_nodes << course
      @skill.save!
    end

    should "have at least one CompetenceNode" do
      assert_not_empty @skill.competence_nodes
    end
